...
# These are the NATIVE implementations. Blocks reach flash through the
# kernel registry (kernels/flash.py), which pins aten or a provider
# generation (FA3 on sm90) per device at resolve time.
...
def flash_fwd(
    ...
    out: torch.Tensor | None = None, lse_out: torch.Tensor | None = None,
) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    t = q.shape[0]
    mq = int(max_seqlen)
    fresh, lse, _rng, _unused, _ = torch.ops.aten._flash_attention_forward(
        q.view(t, n_heads, head_dim),
        k.view(t, n_kv_heads, head_dim),
        v.view(t, n_kv_heads, head_dim),
        cu_seqlens, cu_seqlens, mq, mq, 0.0, bool(causal), False,
        scale=softmax_scale)
    flat = fresh.reshape(t, n_heads * head_dim)
    if out is not None:
        out.copy_(flat)
        flat = out
...
def flash_bwd(
    ...
    cu_seqlens: torch.Tensor, max_seqlen: int,
    causal: bool = True, softmax_scale: float | None = None,
    dq_out: torch.Tensor | None = None, dk_out: torch.Tensor | None = None,
    ...
    dq3, dk3, dv3 = torch.ops.aten._flash_attention_backward(
        ...
        0.0, bool(causal), philox, philox, scale=softmax_scale)
    dq = dq3.reshape(t, n_heads * head_dim)
    dk = dk3.reshape(t, n_kv_heads * head_dim)
    dv = dv3.reshape(t, n_kv_heads * head_dim)
    if dq_out is not None:
        dq_out.copy_(dq)
        dq = dq_out
    if dk_out is not None:
        dk_out.copy_(dk)
        dk = dk_out
