# dataflow_training.blocks — the shared executable library

**Purpose.** The workload's shared task-executable machinery: ops (math
+ reference + costs) compose into block executables resolved by
`compute_block_key`. Family-SPECIFIC blocks live with their families
(`model_families/<family>/blocks.py`); this package holds everything
family-neutral they compose. Imports torch/triton; never imports the
simulator (enforced: `tests/test_import_boundaries.py`).

## Layout

| where | what |
|---|---|
| `base_blocks.py` | family-NEUTRAL executables: `_Base` (private), `EmbedFwd`/`EmbedBwd`, `HeadLoss`, the optimizer-step dispatch (`OptimizerStep`; `AdamWStep` is its back-compat alias), `AdamWHyper`, `RoundPrologue` |
...
| `linear.py` | the linear seam: per-field `LinearFwd`/`LinearDGrad`/`LinearWGrad` triples — blocks never call raw GEMM entry points on weight fields |
| `modules/` | shared building-block modules: the pluggable `moe/` package, `dsa_forms.py`, `mla_forms.py` |
| `layouts.py`, `ops.py`, `optim.py` | packed layouts + dims vocabulary, eager op library, optimizer defs/policies (incl. `freeze`, `register_optimizer`) |

Registry kernel implementations live in the SIBLING package
`dataflow_training/kernels/` (below); torch-buffer interop lives on
the engine side (`dataflow.runtime.interop`).
...
- **Executable**: `launch(TaskContext)` — enqueue device work on
  ...
  documented next to each executable class (family blocks:
  `model_families/<family>/blocks.py`). Full seam spec:
  `docs/program_contract.md`; launch-path rules: `docs/task-contract.md`.
- **Op anatomy** (`ops.py`): every op has a launch form (writes into
  ...
  autograd-able — used by gradcheck. Numerics discipline: bf16 storage,
  fp32 reductions/normalization statistics. (The isolated twins in
  `reference_models/` reimplement the math independently on top.)
- **Layouts** (`layouts.py`): packed offsets for weights / saved context /
  ...
  Import-light: `view()`/`views()` import torch interop lazily.
- **Interop** (`dataflow.runtime.interop`): ctypes-built DLPack capsules
  wrap raw runtime pointers as torch tensors (zero-copy, no ownership);
  `external_stream` routes torch kernels onto runtime streams.
...
`dataflow_training.run.profiling` (allocator peak-delta with runtime-owned
I/O buffers invisible to torch) and written back into programs.
Runtime-owned buffers never come from torch. `torch.compile` is not used
(no workspace introspection; cudagraph pools break external-buffer
ownership).
...
(`dataflow_training.testing`): op backward → block backward (incl.
...
implementations (`dataflow_training/kernels/`) — one file per op family
(`swiglu.py`, `rope.py`, `rmsnorm.py`, `cross_entropy.py`, `adamw.py`,
`muon.py`, ...), each registering an eager fallback (always available)
and a fused Triton default. GEMMs (cuBLAS), flash attention and embed
scatter/gather (aten) stay direct calls.
