"""olmoe presets: the MoE engine-parity smoke twin at the locked training
configuration (``dataflow_training.run.presets`` owns the locked
constants)."""
from __future__ import annotations

from .model import ShapedOlmoeConfig
...
# reference trains pure CE, so the engine must too; LBL-ON engine parity
# is gated separately once the per-step aux machinery lands.
def olmoe_smoke_preset() -> ShapedOlmoeConfig:
