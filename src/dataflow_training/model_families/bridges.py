...
One bridge module per family package (``llama3/bridge.py``,
``qwen3/bridge.py``, ...), each exposing the uniform pair the driver
dispatches on —
...
byte plumbing lives in ``bridge_common``. The engine stores each block's
projection matrices packed ``(in, out)``; references use ``nn.Linear`` (weight
...
(The bridge modules import ``reference_models`` at top level; the
``model_families`` package init arms the repo-root sys.path entry before any
submodule here runs.)
"""
...
from .bridge_common import (  # noqa: F401
...
from .qwen35 import bridge as qwen35_bridge
from .qwen35moe import bridge as qwen35moe_bridge
from .qwen3moe import bridge as qwen3moe_bridge
...
FAMILY_BRIDGES = {
    ...
    "ShapedQwen3MoeConfig": qwen3moe_bridge,
    "ShapedQwen35Config": qwen35_bridge,
    "ShapedQwen35MoeConfig": qwen35moe_bridge,
}
