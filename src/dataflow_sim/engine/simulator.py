...
def _run_impl(
    ...
    *,
    ...
    peak_backing_memory_bytes = initial_alloc["backing"]
    ...
    # triggers). A release fired at-or-after this position for an object with
    # no terminal placement frees the BACKING copy too: the object is dead
    # everywhere, and its host bytes must not haunt the backing capacity
    # (decisive for grad-accum programs whose per-round activations would
    # otherwise accumulate dead backing copies round after round).
    last_ref: dict[str, int] = {}
    for idx, t in enumerate(tasks):
        for obj_id in t.inputs:
            last_ref[obj_id] = idx
        for trig in t.offload_after:
            last_ref[trig.obj_id] = idx
    ...
    for i, task in enumerate(tasks):
        ...
        if task.releases_after:
            for obj_id in task.releases_after:
                ...
                # dead everywhere: last reference is behind us and no terminal
                # placement claims the bytes -> the backing copy frees too
                if i >= last_ref.get(obj_id, -1) and obj_id not in chain.final_locations:
                    backing_key: PoolKey = (obj_id, "backing")
                    backing_entry = pool.get(backing_key)
                    if backing_entry is not None and backing_entry.state == "live":
                        loc_total["backing"] -= backing_entry.size
                        del pool[backing_key]
            emit(
                "release",
                end_t,
                i + 1,
