...
Current modular chains annotate the fused `head_loss` task as the head
release point. Initial compute residency added: W_0..W_{w-1}, W_head.
...
def _head_id(mode: _Mode) -> str:
    return "head" if mode == "legacy" else "head_loss_0_0"
...
def _annotate_backward(
    ...
    i = _layer_idx(task, mode)
    upstream = task.inputs[0]  # dy_head or dy_{i} (consumer-indexed)
