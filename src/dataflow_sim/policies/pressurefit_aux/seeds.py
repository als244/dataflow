...
def _pressure_initial_placement(
    ...
    *,
    ...
    ``mode="greedy"`` fills spare capacity beyond task 0's inputs with the
    earliest-used backing objects (cold-miss aware). ``mode="task0"`` stops
    at task 0's inputs: initial placement is not free for executors that
    replay the chain (they upload it synchronously before t=0), so every
    other arrival should be a planned prefetch the schedule can overlap.
    """
    if not bare.tasks:
        return set()
...
def _initial_residency(
    ...
    # sorted: iterating a STRING set varies with per-process hash
    # randomization, which seeded the intervals dict in random order and
    # cascaded through every downstream tie-break — same chain and cap
    # produced different (equal-quality) plans per process. One sort makes
    # the whole pipeline byte-deterministic again.
