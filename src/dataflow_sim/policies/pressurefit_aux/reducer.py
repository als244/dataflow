...
class _PressureReducer:
    ...
    def __init__(
        ...
        # 98% of near-floor planning time (profiled: 412M _effective_a calls
        # for one 8B plan). Both terms are per-interval properties — an
    ...
    def _relaxed_terms_add(self, oid: str, iv: tuple[int, int], sign: int) -> None:
        ...
        if fire is not None and 0 <= fire < facts.n - 1 and _effective_a(a, p) <= fire <= b:
            self.departing[fire + 1] += size
        if a > -1 and a != p:
            boundary = _effective_a(a, p)
    ...
    def _apply_split(self, split: _SplitOption) -> None:
        ...
        self._relaxed_terms_add(split.oid, (a, b), -1)
        for piece in pieces:
