...
_DTYPE_BYTES = {
    ...
    "fp32": 4,
    "float32": 4,
    # integer ids (token/target streams)
    "int32": 4,
    "int64": 8,
}
...
class TrainingConfig:
    ...
    # "interleaved" (default, matching the runtime since M4.9): each
    # optimizer task is emitted right after the LAST mutation of its
    # gradient (final round's backward), so its state streaming overlaps
    # the remaining backward. "tail" restores the legacy all-optimizers-
    # after-all-rounds order (a measured 7-8% step tax at seq-1K/8B).
    # Task ids are identical in both modes; only order changes.
