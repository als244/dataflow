...
class PlannerParams(BaseModel):
    ...
    # host/backing capacity; None = unlimited. A finite cap constrains
    # offloading, which pushes the recompute planner toward recomputation.
    backing_memory_capacity_gb: float | None = Field(None, gt=0)
...
def _run_config(
    ...
    bare = workload.chain
    if params.planner.backing_memory_capacity_gb is not None:
        bare = replace(
            bare,
            backing_memory_capacity=int(params.planner.backing_memory_capacity_gb * 1e9),
        )
        workload = replace(workload, chain=bare)
    diagnostics = None
...
def _plan_with_recompute(
    ...
    def cap_backing(chain: TaskChain) -> TaskChain:
        # finite backing makes save-heavy variants infeasible in simulation,
        # steering the selector toward recomputation — same admission
        # semantics as the runtime's d2h queue head
        if planner.backing_memory_capacity_gb is None:
            return chain
        return replace(
            chain,
            backing_memory_capacity=int(planner.backing_memory_capacity_gb * 1e9),
        )

    def build_variant(levels) -> TaskChain:
        return cap_backing(model.build_training_workload(
            cfg,
            hw,
            input_shape=input_shape,
            dtype_policy=dtype_policy,
