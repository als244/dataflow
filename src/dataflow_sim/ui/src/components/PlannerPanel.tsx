...
export function PlannerPanel({
  ...
  const [backingBudgetText, setBackingBudgetText] = useState(
    params.planner.backing_memory_capacity_gb === null
      ? ""
      : String(params.planner.backing_memory_capacity_gb),
  );
  useEffect(() => {
    setBackingBudgetText(
      params.planner.backing_memory_capacity_gb === null
        ? ""
        : String(params.planner.backing_memory_capacity_gb),
    );
  }, [params.planner.backing_memory_capacity_gb]);
  ...
  return (
    <div className="panel planner-panel">
      ...
      <div className="form-grid planner-grid">
        ...
        <label className="form-field">
          <span className="form-field-label">Slow Memory Budget (GB)</span>
          <input
            type="number"
            min={0.000001}
            step="any"
            placeholder="Unlimited"
            disabled={controlsLocked}
            value={backingBudgetText}
            onChange={(e) => {
              const text = e.target.value;
              setBackingBudgetText(text);
              if (text === "") {
                setPlanner("backing_memory_capacity_gb", null);
                return;
              }
              const v = Number(text);
              if (Number.isFinite(v) && v > 0) setPlanner("backing_memory_capacity_gb", v);
            }}
          />
        </label>
      </div>
