...
export interface PlannerParams {
  ...
  backing_memory_capacity_gb: number | null;
  recompute: boolean;
}
...
export const DEFAULT_PARAMS: SimulationParams = {
  ...
  planner: {
    ...
    backing_memory_capacity_gb: null,
    recompute: true,
  },
};
...
function FormSubsection({
  ...
  if (collapsible) {
    // closed by default: dense parameter grids read as noise when always
    // expanded; the summary line keeps the current values scannable
    return (
      <details className="form-subsection form-subsection-collapsible">
        <summary className="form-subsection-title">
          {title}
          {summaryHint ? <span className="form-subsection-hint">{summaryHint}</span> : null}
        </summary>
        {children}
      </details>
    );
        ...
        <section className="form-section">
          ...
          {modelTrainingWorkload ? (
            <>
              ...
              <FormSubsection
                title="Model Architecture Dimensions"
                collapsible
                summaryHint={`${modelTrainingWorkload.model.family} · ${modelTrainingWorkload.model.n_layers}L · d${modelTrainingWorkload.model.d_model}`}
              >
                <div className="form-row">
                  <label className="form-field form-field-wide">
              ...
              <FormSubsection
                title="Datatypes"
                collapsible
                summaryHint={`${modelTrainingWorkload.datatypes.weight_dtype} wt · ${modelTrainingWorkload.datatypes.activation_dtype} act · ${modelTrainingWorkload.datatypes.compute_precision} compute`}
              >
              ...
              <FormSubsection
                title="Data Sizing"
                collapsible
                summaryHint={`seq ${modelTrainingWorkload.training.seqlen} · ${modelTrainingWorkload.training.num_seqs} seqs · ga ${modelTrainingWorkload.training.grad_accum_rounds}`}
              >
                <div className="form-grid">
                  <label className="form-field">
              ...
              <FormSubsection
                ...
                collapsible
                summaryHint={`${modelTrainingWorkload.training.optimizer} · ${modelTrainingWorkload.training.num_steps} steps`}
              >
                <div className="form-grid">
                  <label className="form-field">
              ...
              <FormSubsection
                ...
                collapsible
                summaryHint={modelTrainingWorkload.training.final_model_state_on_backing
                  ? "final state → slow memory"
                  : "final state stays on fast"}
              >
                <label className="form-field form-field-checkbox">
        ...
        <section className="form-section">
          ...
          {showParallelismControls && modelTrainingWorkload && (
            <FormSubsection
              title="Parallelism"
              collapsible
              summaryHint={`ep ${modelTrainingWorkload.parallelism.ep_group_size}`}
            >
              <div className="form-grid">
                <label className="form-field">
          ...
          {HW_FIELD_SECTIONS.map(({ title, fields }) => (
            <FormSubsection key={title} title={title} collapsible>
              <div className="form-grid">
                {fields.map((f) => {
