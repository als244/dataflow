"""Structural gates for the shaped Llama3 program builder: program
validation, object-size totals, gradient-accumulation mutation
patterns, recompute variants, optimizer placement, tied embeddings,
and per-layer-kind lowering.

Tests:
- test_tiny_validates: the tiny shaped config builds a program that passes validate_program.
- test_8b_shape_totals: the 8B build's param/optimizer/saved-context byte totals and task count match the expected chain structure.
- test_grad_accum_mutation_pattern: round 0 creates each layer's dW and a later round mutates it in place, and the optimizer mutates W and O.
- test_recompute_variant_moves_A_production: a recompute level moves a layer's saved-context production from block_fwd to block_recompute, leaving other layers untouched.
- test_rewrites_cover_all_saved_contexts: recompute_rewrites target exactly the per-layer saved-context objects.
- test_interleaved_optimizer_fires_at_final_grad_mutation: default placement puts each optimizer right after its grad's final mutation with the same task set as tail mode and embed's optimizer closing the step, while tail mode keeps the optimizer suffix.
- test_interleaved_optimizer_respects_reader_ordering: no task after optimizer_i reads W_i/O_i within the step and the consumed grad is never referenced again.
- test_tied_embeddings_chain_structure: tied embeddings share one W_embed/O_embed for embed and head, drop head objects and optimizer_head, accumulate the shared dW_embed, and plan+simulate green.
- test_heterogeneous_kinds_emit_per_kind_keys_and_sizes: a per-layer-kind spec keeps task ids uniform while compute-block keys, W/A sizes, and rewrite keys follow each layer's kind.
"""
import pytest
...
def test_heterogeneous_kinds_emit_per_kind_keys_and_sizes():
    ...
    from dataflow_training.lowering.shaped_program import LayerKindSpec, build_shaped_program
