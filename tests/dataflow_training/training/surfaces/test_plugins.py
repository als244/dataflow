...
Tests:
- test_explicit_plugin_load_end_to_end: an explicitly loaded plugin module registers its family, resolves by exact config type, lowers, exposes its bench preset by name, and passes the contract validator.
- test_entry_point_discovery: load_plugins with no flags discovers and registers a family declared via the dataflow.families entry point.
- test_validate_family_reports_broken_surface: validate_family flags a family whose config type is not a dataclass.
- test_register_family_rejects_duplicates: register_family raises when the name is already taken.
"""
