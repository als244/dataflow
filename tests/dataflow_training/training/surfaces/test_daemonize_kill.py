...
Tests:
- test_kill_terminates_and_verifies: --kill tears down the daemon group, exits 0, consumes the pidfile, and a repeat --kill is an idempotent no-op reporting nothing to do.
- test_kill_escalates_past_sigterm: against a SIGTERM-ignoring child, --kill escalates to a hard kill, exits 0 reporting the escalation, and the group is gone.
"""
import os
import subprocess
import sys
...
pytestmark = pytest.mark.skipif(
    os.name != "posix",
    reason="POSIX-only: process groups, os.killpg, and fork-based daemonize")
