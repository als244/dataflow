...
Tests:
- test_causal_mask_does_not_leak_across_batch: batched attention over two uniform sequences equals the two per-sequence attentions concatenated.
- test_batch_ga_model_step_matches_reference: a batch>1, two-round grad-accumulation engine step matches the reference twin that sums the per-round losses then steps, with final params compared in twin-name space through the bridge.
"""
import pytest
