...
Tests:
- test_flash_ragged_matches_reference_autograd: per-seq flash forward/backward under ragged packing matches autograd through the block-diagonal reference.
- test_llama3_model_step_ragged_matches_golden: a llama3 ragged model step matches the golden reference.
- test_qwen35_model_step_ragged_matches_golden: the qwen3.5 hybrid family's ragged step (DeltaNet and conv state resets via cu_seqlens) matches the golden.
- test_varlen_single_launch_matches_reference_autograd: the single-launch cu_seqlens flash path, with a short final segment, matches naive autograd through the block-diagonal reference.
- test_model_step_ragged_matches_golden_all_families: for each of the remaining seven families, a ragged model step matches the golden.
"""
import pytest
...
def test_model_step_ragged_matches_golden_all_families(family):
