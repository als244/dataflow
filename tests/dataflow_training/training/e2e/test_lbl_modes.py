...
Tests:
- test_retained_structure_and_default_stability: retained mode feeds every round's AuxTemp pack into the last block backward while earlier blocks and the knob-off build see only their own round.
- test_modes_agree_on_router_at_ga1_and_differ_upstream: at ga=1 both code paths produce the same router, but the per-round path's upstream gradients differ from retained's.
- test_modes_differ_on_router_at_ga4: at ga=4 the per-round and retained routers no longer coincide.
- test_retained_router_delta_is_ga_invariant_per_round_is_not: retained's aux router gradient is invariant across round partitionings while the per-round default's gap is many times larger.
- test_counts_ga_invariant_and_sum_exact: expert-count totals equal T_STEP*top_k exactly and per-expert counts agree across partitionings within a small flip budget.
- test_counts_overall_monotone_across_steps: expert_counts_overall accumulates both steps' assignments and stays at least the current step's counts.
- test_recompute_never_double_counts: a recompute-everything plan yields the same one-step count total as save-all, with only bounded near-tie flips.
- test_noaux_bias_ga_invariant_bit_exact: the no-aux router bias vector is bit-identical across ga partitionings and non-zero.
"""
