...
Precedence has two modes. LOGICAL-view targets ("all", bare ids)
...
view: they use only that writer's own snapshot — the self-sufficiency
contract — and land shard bytes in a LOCAL object at local
coordinates.

Tests:
- test_all_targets_pick_authoritative_sources: "all" resolves every logical byte exactly once, preferring authoritative slices, into per-snapshot remap plans in restore's wire shape.
...
def test_all_targets_pick_authoritative_sources():
    record = build_record(base_record_kwargs())
    plan = resolve_targets(record, "all")
    steps = plan_by_snapshot(plan)

    # W_0 is covered twice; only writer 0's AUTHORITATIVE copy is used
    ...
    # O_0's two authoritative shards each contribute their half
