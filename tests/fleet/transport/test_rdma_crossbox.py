"""Cross-box rdma-host gates over the real cross-box link: RDMA_WRITE
...
Tests:
- test_rdma_crossbox_byte_identity: a large object RDMA_WRITEs to the remote slab and the remote reads back the exact bytes (length and sha256 match).
- test_rdma_crossbox_reverse: the remote sends an object back and it lands byte-identical in the local store.
...
if roce_v2_ipv4_gid(TOPO.local().ib_dev) is None:
    pytest.skip(f"no ACTIVE RoCE v2 GID on {TOPO.local().ib_dev}",
                allow_module_level=True)
...
def test_rdma_crossbox_throughput_matches_probed_bw(rig):
    ...
    print(f"\nrdma cross-box: 256 MiB in {dt:.3f}s "
