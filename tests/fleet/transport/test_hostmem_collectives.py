"""Hostmem collectives between two REAL daemons sharing this
...
Tests:
- test_allreduce_matches_fp32_reference_and_is_replica_identical: a two-rank allreduce equals the fp32-accumulated reference and both replicas are bit-identical.
- test_broadcast_root_bytes_land_everywhere: broadcast copies the root's bytes verbatim into the non-root rank's buffer.
- test_reduce_scatter_all_gather_roundtrip_matches_allreduce: reduce_scatter hands each rank its shard of the reference sum, all_gather reassembles the full reduction, and rs+ag equals allreduce bit-for-bit.
- test_back_to_back_ops_fifo: four queued allreduces per rank complete in FIFO order with the correct per-op sums.
- test_stream_parks_until_worker_releases: with one rank posted the group stream stays parked (not query-ready) until the second rank joins, then the sum lands (skipped without wait-value support).
"""
import threading
import time
...
def test_reduce_scatter_all_gather_roundtrip_matches_allreduce(rig):
    ha, hb = rig["ha"], rig["hb"]
