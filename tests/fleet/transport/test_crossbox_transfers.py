...
Tests:
- test_crossbox_send_byte_identity: a 32 MiB object sent local->remote arrives with matching byte length and sha256 on the remote daemon.
- test_crossbox_reverse_direction: the remote daemon initiates a send back over the same bidirectional link; the local side receives the exact bytes and records the write as coming from the peer.
- test_crossbox_throughput_matches_probed_bw: a 256 MiB zero-copy send completes within the time implied by the connect-time bandwidth probe (no hardcoded speed), so a hidden copy shows up as a miss.
...
def test_crossbox_send_byte_identity(rig):
...
def test_crossbox_throughput_matches_probed_bw(rig):
    ...
    print(f"\nsocket cross-box: 256 MiB in {dt:.2f}s "
