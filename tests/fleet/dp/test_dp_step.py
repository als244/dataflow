"""The FIRST comm-in-task data-parallel step — two real
...
Asserts, in order of strength:
  ...
  2. both match a SINGLE-ENGINE run over the combined batch (same
     ...
     accumulation-tree noise — the single-step DP equivalence.

Tests:
- test_two_daemon_dp_step_replicas_bitwise_and_match_single_engine: two daemons each train their half of the batch with the global valid-token denominator; the per-rank partial losses differ, both replicas' weights end bitwise identical, and they match the single-engine combined-batch run within rel_l2 3e-3.
"""
import threading
import time
...
def test_two_daemon_dp_step_replicas_bitwise_and_match_single_engine(
    ...
    try:
        ...
        # on the rank-complete artifact) compile + cuModuleLoad every
        # kernel; a FIRST launch during a parked collective deadlocks
        # the device. Re-seed afterwards — and RE-PUT the data: the
        # init program refills EVERY initial object, token buffers
        # included (the trap that silently gave both ranks identical
        # seeded tokens on the first attempt).
        for rank, client in ((0, ca), (1, cb)):
        ...
        for t in threads:
            t.start()
        join_threads(threads, 60, label="two-rank DP step")
        for r in runs:
        ...
        for i, layer in enumerate(fl.layers):
            ...
            for f in layer.weights.fields:
                ...
                worst = max(worst, d)
        print(f"\n[dp-step] DP(2 ranks) vs single-engine: worst field "
              f"rel_l2 = {worst:.3e}")
