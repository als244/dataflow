"""Sharded optimizer == plain data parallelism, BITWISE.
...
Tests:
- test_zero1_bitwise_equals_plain_dp: with a non-degenerate shard plan, the zero1 sharded lowering and plain replicated DP produce identical per-step per-rank losses, zero1 replicas bitwise identical to each other and to the plain weights, and every sharded O object shrinks (total under 0.75x plain).
"""
import threading
import time
