...
Tests:
- test_invalid_chain_rejected: each make_invalid_* fixture raises ValidationError before stepping (or runs cleanly when its expected keyword is None), with the message matching the declared keyword.
- test_validate_can_be_skipped: run(chain, validate=False) bypasses the static prepass so an unknown-input chain does not raise ValidationError.
"""
import pytest
