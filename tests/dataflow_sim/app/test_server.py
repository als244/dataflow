"""Web server endpoints: simulate, preview_workload, and presets.
...
- test_simulate_omits_policy_diagnostics_for_other_policies: simulate returns no policy diagnostics for non-PressureFit policies.
- test_simulate_large_chain_uses_snapshot_free_response: a large chain returns a snapshot-free response with no events, a memory trace, and thousands of intervals.
- test_simulate_final_model_state_on_backing_is_opt_in: final model state on backing is opt-in, leaving final_locations empty by default and pinning weights and optimizer state to backing when set.
- test_preview_model_training_workload_returns_dataflow_schema: previewing a model-training workload returns a dataflow/v1 schema plus chain, compute blocks, and breakdown.
- test_preview_accepts_uploaded_schema_and_returns_bare_chain: previewing an uploaded schema returns the bare chain (no releases) and its single-instance compute block.
- test_presets_include_only_public_model_workloads: the presets endpoint lists exactly the public model workloads, sorted, with expected families, datatypes, and per-family field and capability sets.
- test_preview_accepts_datatype_policy_and_exports_metadata: a supplied datatype policy is exported into the schema metadata's dtype_policy.
- test_presets_include_sram_accelerator_hardware: the presets include the SRAM Accelerator hardware spec with its exact values.
- test_presets_include_gb300_hardware: the presets include the GB300 hardware spec with its exact values.
- test_h100_marks_fp4_matmul_as_unsupported: the H100 preset marks fp4 matmul unsupported (null fp4 peak and efficiency) with a 400 GB/s scale-up link.
- test_simulate_uploaded_schema_workload: simulating an uploaded schema echoes its name and first task and reports makespan at least its fixed cost with zero token throughput.
"""
...
def test_simulate_final_model_state_on_backing_is_opt_in():
    ...
    assert finalized_body["chain"]["final_locations"] == {
        ...
        "W_embed": "backing", "O_embed": "backing",
