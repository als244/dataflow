"""Generic DataflowProgram schema: realization, preview, normalization, and validation.

Tests:
- test_generic_program_realizes_to_valid_task_chain: a generic DataflowProgram realizes to a valid task chain with scaled bandwidths, propagated roles, and a cost above its fixed term.
- test_preview_counts_roles_and_groups: the program preview reports task and group counts, per-role bytes, and compute-block instance counts.
- test_inline_cost_normalizes_to_one_off_compute_block: normalization turns an inline task cost into a one-off compute block keyed inline:<task>.
- test_compute_block_summary_reports_total_effective_tflops: a compute block's effective_tflops equals its effective flops over its runtime.
- test_scale_up_roofline_uses_scale_up_bandwidth_without_memory_efficiency: a scale_up roofline term uses scale-up bandwidth and ignores memory efficiency in its timing.
- test_unsupported_fp4_matmul_hardware_fails_clearly: realizing an fp4 matmul on hardware lacking fp4 support raises a clear FP4 error.
- test_repeated_tasks_share_compute_block_summary: two tasks sharing a compute-block key collapse into one block summary with instance_count 2.
- test_metrics_preview_and_summary_metadata: declared metrics surface in both the preview and the realized workload summary metadata.
- test_program_validation_errors_are_specific: each malformed-program patch raises a ValidationError whose message names the specific violation.
"""
import pytest
