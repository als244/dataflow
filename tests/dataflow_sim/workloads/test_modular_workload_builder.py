"""Modular workload builder: dtype policy, op formulas, family presets, and program lowering.

Tests:
- test_dtype_policy_defaults_and_low_precision_sizes: the default DTypePolicy is bf16 (fp8 indexer) and low-precision element sizes and packed TensorRef bytes compute correctly.
- test_muon_uses_split_real_matrix_shapes_and_expert_counts: Muon optimizer ops use split rather than fused gate/up matrix shapes and carry per-expert counts.
- test_module_optimizer_matrices_use_real_parameter_shapes: each module's optimizer matrices expose the real parameter names, shapes, expert counts, and total element count.
- test_matmul_accumulate_epilogue_bytes: matmul memory bytes account for the accumulate epilogue (extra output read) on fused-forward and weight-grad ops.
- test_backward_matmul_policy_separates_activation_and_parameter_gradients: backward matmul byte counts follow a mixed dtype policy separating activation- and parameter-gradient lanes.
- test_datatype_policy_changes_program_bytes_and_compute_precision: a low-precision dtype policy shrinks weight/optimizer/activation bytes (int32 tokens invariant) and sets matmul efficiency lanes per dtype.
- test_expert_dispatch_dtype_changes_forward_and_backward_dispatch_lanes: an fp8 expert-dispatch dtype shrinks only the dispatch-lane ops in forward and backward, leaving the others unchanged.
- test_ep_group_size_shards_routed_experts_and_uses_scale_up_movement: an ep_group_size of 2 shards routed-expert weights and grads and switches scatter/gather to scale-up movement while conserving total expert flops.
...
- test_gpt_oss_modules_emit_expected_subop_chains_and_block_keys: GPT-OSS sliding- and full-attention MoE blocks emit the expected subop chains and named block keys.
- test_nemotron_modules_emit_expected_subop_chains_and_dtype_lanes: Nemotron-H mamba/attention/MoE blocks emit the expected subop chains, named block keys, and fp8-dispatch byte lanes.
- test_training_program_uses_model_order_reverse_backward_and_optimizer: the training program orders embed/forward/head/reverse-backward/optimizer tasks correctly for interleaved and tail placements, with the right backward inputs, fused head block, and final locations.
- test_tied_embeddings_share_one_weight_grad_and_optimizer: tied embeddings share one weight/grad/optimizer-state set (no separate head weight or step), with the head creating and embed_bwd accumulating the shared grad.
- test_lm_head_optimizer_defaults_to_adamw_for_non_none_layer_optimizer: the LM head defaults to an AdamW optimizer step for a non-none layer optimizer and to none when the layer optimizer is none.
- test_moe_recompute_variant_rewires_activation_producer_and_block_metadata: an MoE recompute variant rewires the activation to a recompute block and reports recompute and optimizer block metadata with split gate/up Muon counts.
- test_varied_family_workloads_run_and_report_kpis: every family workload simulates to positive makespan, throughput, and effective-tflops KPIs with the expected head and embedding blocks.
- test_constrained_memory_recompute_planning_selects_useful_variants: under a constrained memory cap the recompute planner selects useful variants that beat the baseline makespan and report positive recompute percentage.
"""
...
def test_datatype_policy_changes_program_bytes_and_compute_precision():
    ...
    # token ids are int32 streams: dtype-policy-invariant
    assert bf16_objects["tokens_0_0"] == training.tokens * 4
    assert lowp_objects["tokens_0_0"] == training.tokens * 4
    ...
    assert lowp_outputs["dy_0_0_1"] == training.tokens * model.dims.d_model
    ...
    assert split_grad_outputs["dy_0_0_1"] == training.tokens * model.dims.d_model * 2
...
def test_expert_dispatch_dtype_changes_forward_and_backward_dispatch_lanes():
    ...
    assert dispatch_outputs["dy_0_0_0"] == bf16_outputs["dy_0_0_0"]
...
def test_training_program_uses_model_order_reverse_backward_and_optimizer():
    ...
    # the runtime's true round anatomy: embed_fwd opens each round,
    # embed_bwd closes it; the head is ONE fused head_loss task
    ...
    # interleaved (default): each optimizer lands right after the LAST
    # mutation of its gradient in the final round; the embedding
    # optimizer closes the step
    assert [task.id for task in program.tasks[-8:]] == [
        "head_loss_0_1",
        "step_0_head",
        "b_0_1_1",
    ...
    tail = Llama3ForTraining(config).build_training_program(
        TrainingConfig(
            ...
            grad_accum_rounds=2,
            optimizer="adamw",
            optimizer_placement="tail",
            final_model_state_on_backing=True,
        )
    )
    assert [task.id for task in tail.tasks[-5:]] == [
        "embed_bwd_0_1",
        "step_0_embed",
        "step_0_0",
        "step_0_1",
    ...
    assert {task.id for task in tail.tasks} == {task.id for task in program.tasks}
    assert not any(task.id.startswith("r_") for task in program.tasks)
    # backward reads [upstream dy, saved ctx, its forward input x, W (+dW)]
    assert tasks["b_0_1_1"].inputs == [
        "dy_0_1_1", "A_0_1_1", "y_0_1_0", "W_1", "dW_0_1",
    ]
    ...
    blocks = _blocks_by_key(program)
    # fused head: ONE compute block carrying the fwd+bwd subop chain
    assert blocks["lm_head.head_loss"].name == "LM Head + Loss (fused)"
    assert [op.name for op in blocks["lm_head.head_loss"].subops] == [
        "final_norm",
        "head_proj",
        "cross_entropy",
        "head_proj_dgrad",
    ...
    assert _block_keys(program) >= {
        "embedding.forward",
        "embedding.backward",
        "embedding.optimizer_step.adamw",
...
def test_tied_embeddings_share_one_weight_grad_and_optimizer():
    from dataclasses import replace as dc_replace

    from dataflow_sim.workloads.training_builder import TrainingBuilder
    ...
    tied_spec = dc_replace(model.embedding, tied=True)
    builder = TrainingBuilder(
        family_name=model.family_name,
        metadata_kind=model.metadata_kind,
    ...
    program = builder.build_training_program(
        TrainingConfig(
            seqlen=128, num_seqs=1, grad_accum_rounds=2, optimizer="adamw",
        )
    )
    tasks = _tasks_by_id(program)
    initial_ids = {obj.id for obj in program.objects}

    # ONE weight/state pair serves both ends of the model
    assert "W_embed" in initial_ids and "O_embed" in initial_ids
    assert "W_head" not in initial_ids and "O_head" not in initial_ids

    # head_loss reads the shared weight and CREATES the shared grad in
    # round 0; embed_bwd accumulates into it (round 0 included)
    assert tasks["head_loss_0_0"].inputs == ["y_0_0_1", "targets_0_0", "W_embed"]
    assert [o.id for o in tasks["head_loss_0_0"].outputs][-1] == "dW_embed_0"
    assert tasks["embed_bwd_0_0"].mutates == ["dW_embed_0"]
    assert tasks["head_loss_0_1"].mutates == ["dW_embed_0"]

    # a single embedding optimizer covers both; no separate head step
    assert "step_0_head" not in tasks
    assert tasks["step_0_embed"].inputs == ["dW_embed_0", "W_embed", "O_embed"]
    assert tasks["step_0_embed"].mutates == ["W_embed", "O_embed"]
    assert program.tasks[-1].id == "step_0_embed"
...
def test_varied_family_workloads_run_and_report_kpis():
    ...
    for model, training in cases:
        ...
        assert {"lm_head.head_loss", "embedding.forward", "embedding.backward"} <= block_keys
