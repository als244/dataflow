"""Core simulator semantics: events, memory accounting, transfers, and stalls.

Tests:
- test_single_task_emits_start_then_end: a single task emits task_start then task_end at the right times with a matching interval.
- test_snapshot_free_run_keeps_intervals_without_events: snapshots=False yields no events and an empty memory trace but the same peak memory and task intervals as a full run.
- test_snapshot_free_run_can_emit_compact_memory_trace: snapshots=False with memory_trace=True emits a compact memory trace (no events) with nonzero banded byte counts.
- test_output_reserved_at_start_visible_at_end: an output is reserved and counted at task start and becomes live at task end.
- test_releases_after_emits_release_event_and_frees_memory: releases_after emits a release event that frees the object's memory.
- test_missing_input_raises: a task input absent from memory raises 'not present' under validate=False.
- test_output_visible_only_for_downstream_not_self: a task cannot consume its own declared output as an input, raising 'not present'.
- test_output_collision_raises: an output colliding with an existing object id raises 'collides'.
- test_release_nonexistent_raises: releasing an object that does not exist raises 'cannot release'.
- test_reference_stream_in_snapshot_tracks_next_use: snapshot reference streams track an object's next use as time advances across task boundaries.
- test_reference_stream_in_snapshot_includes_future_outputs: a future output's first reference in the snapshot is its producer, not the later consumer.
- test_location_propagates_to_memory_entries: object and output locations propagate to the memory entries in a snapshot.
...
- test_offload_emits_transfer_events_and_frees_compute: an offload emits enqueue/start/end transfer events and leaves the object live on backing, gone from fast.
- test_prefetch_emits_transfer_events_and_lands_live_on_compute: a prefetch emits from_slow transfer events and lands the object live on both backing and fast.
- test_compute_stalls_waiting_on_inbound_prefetch: a consumer stalls until the inbound prefetch it needs completes.
- test_compute_waits_for_needed_queued_inputs_not_unneeded_tail: a task starts once its needed queued inputs are live, not after later unneeded queue entries.
- test_compute_stalls_waiting_on_fast_memory_capacity_to_free: a task stalls until an offload frees fast memory for its output.
- test_offload_deadlocks_when_backing_memory_capacity_too_tight: an offload that can never fit on backing stays queued and raises at end of run.
- test_prefetch_deadlocks_when_fast_memory_capacity_too_tight: a prefetch that can never fit on fast memory stays queued and raises at end of run.
- test_transfer_queue_serializes_multiple_offloads: two offloads from one task serialize on the to_slow stream, the second starting when the first ends.
- test_per_trigger_runtime_overrides_bandwidth: a per-trigger runtime override replaces the bandwidth-derived transfer duration.
- test_missing_bandwidth_raises_when_used: a transfer with no bandwidth and no override raises 'bandwidth'.
- test_offload_overwrites_existing_backing_copy: an offload over a live backing copy keeps it live until start, flips it to inbound during transfer, and lands live with no new backing bytes.
- test_offload_overwrite_size_mismatch_raises: offloading over a backing copy of a different size raises 'size mismatch'.
- test_prefetch_already_on_compute_raises: prefetching an object already on fast raises 'fast copy already exists'.
- test_prefetch_defers_when_source_still_offloading: a prefetch fired while the source's offload is in flight defers, then auto-enqueues when the offload completes.
- test_prefetch_defers_when_backing_source_pending_inbound: a prefetch fired while the source's offload is still queued (pending) defers.
- test_truly_absent_backing_source_still_raises: prefetching an object with no backing source and none in flight raises 'no recoverable backing source'.
- test_queued_prefetch_consumes_no_compute_bytes_until_transfer_starts: a prefetch queued behind another consumes no fast bytes until its own transfer starts.
- test_queued_offload_consumes_no_backing_bytes_until_transfer_starts: an offload queued behind another consumes no backing bytes until its own transfer starts.
- test_prefetch_blocks_at_start_until_compute_release_frees_bytes: a capacity-blocked prefetch starts only once a downstream release frees the needed fast bytes.
- test_repeated_transfers_of_same_object_get_unique_task_ids: re-prefetching or re-offloading the same object yields distinct transfer task ids via a #N suffix on repeats.
"""
import pytest
