"""Reference-stream construction: first-use ordering over a task list.

Tests:
- test_next_ref_finds_first_appearance: next_ref_time returns the first task-start time an object appears as an input, or None when it never does.
- test_compute_reference_stream_includes_every_input_first_use: the reference stream carries one entry per object at its first input use, with the referencing task and time.
- test_compute_reference_stream_includes_outputs_at_producer_time: an object first allocated as an output references at its producer's start time, before any later read.
- test_compute_reference_stream_terminal_output_appears: an output never consumed downstream still appears in the stream, referenced by its producer.
- test_compute_reference_stream_returns_only_first_use: an object used by several tasks yields a single stream entry, keyed to its first user.
"""
