"""Stall-report attribution and recompute planning over simulator chains.

Tests:
- test_stall_report_attributes_input_wait_to_object: the stall report attributes a task's input-wait stall to the specific object it waited on.
    ...
    for task in chain.tasks:
        ...
        if match is not None:
            ...
            if (k, j, i) not in real_recompute:
                in_act = f"y_embed_{k}_{j}" if i == 0 else f"y_{k}_{j}_{i - 1}"
                tasks.append(
                    Task(
                        id=f"r_{k}_{j}_{i}",
