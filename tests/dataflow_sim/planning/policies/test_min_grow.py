...
- test_min_plan_output_merges_with_downstream_input: the MIN plan merges an output's producer boundary with a later consumer into one contiguous interval.
- test_min_plan_separates_non_adjacent_uses: the MIN plan keeps two non-adjacent uses of one object as two separate intervals.
- test_max_plan_pre_places_backing_init_with_a_minus_1: the MAX plan pre-places every backing-init object with interval start -1.
- test_max_plan_releases_after_last_use: the MAX plan's interval end for an object is its last-use task index.
- test_max_plan_produced_starts_at_producer: for a produced object the MAX plan's interval starts at the producer task index.
- test_max_plan_mutated_grad_exits_at_mutator: a backing-init object mutated only at task k gets a MAX interval ending at k, offloading right after mutation.
- test_respects_static_cap_passes_with_unlimited: the static-cap pre-filter passes a MIN plan under unlimited capacity.
- test_respects_static_cap_includes_next_outputs_reservation: the static-cap check counts the next task's output reservation, failing at cap 14 and passing at 15.
- test_enumerate_reductions_generates_shrink_for_extended_interval: reduction enumeration offers a shrink-leftward candidate for a pre-placed multi-boundary interval.
- test_enumerate_reductions_generates_split_for_gappable_interval: reduction enumeration offers a split candidate for an object with a gappable internal boundary.
- test_derive_schedule_pre_places_backing_init_with_a_minus_1: schedule derivation pre-places a backing-init object whose interval starts at -1.
- test_derive_schedule_emits_prefetch_for_non_pre_placed_backing_init: schedule derivation emits a prefetch for a backing-init object that is not pre-placed.
- test_derive_schedule_mutated_backing_init_offloads_at_mutator: schedule derivation offloads a mutated backing-init object at its mutating task.
- test_derive_schedule_released_after_last_use: schedule derivation releases a non-mutated backing-init object at its last-use task.
- test_score_with_peak_returns_both_values: scoring a plan returns a positive makespan and a positive peak.
- test_infeasible_raises_with_forced_footprint_exceeding_cap: a forced footprint exceeding the cap raises an infeasible ValueError.
