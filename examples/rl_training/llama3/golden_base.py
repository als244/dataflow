...
dataflow.models.llama3_reference): the example is self-contained client
code and carries its own ground-truth trainer; kept in sync by the
example's parity gate.

Golden llama3 reference: plain eager torch + autograd, hand-written.
It shares the packed-weight layouts (so state is directly comparable
field-by-field) but is otherwise independent of the tasks layer's launch
code: forward math is composed from the ops' *reference* forms and
autograd derives backward — catching errors in hand-written backward
compositions.

Parameters are PER-FIELD leaves at each field's storage dtype (the dims'
dtype policy); the AdamW update replicates ops.adamw_step exactly,
including the storage-dtype round-trips of the moments (opt dtype) and
the gradient (grad dtype), so post-step state is structurally comparable
to the runtime's per-field optimizer.
"""
